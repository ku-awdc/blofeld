...
diffusion$setup(graph, beta_freq=0.25, beta_carc=0.005, beta_dens=0.0005)
...
yrs <- YEARS
output <- expand_grid(Year = 1:yrs, Day = 1:365, Spillover = NA_real_, Susceptible = NA_integer_, Infected = NA_integer_, Carcass = NA_integer_, Dead = NA_integer_) |>
  arrange(Year, Day)
...
mkplt <- function(wbpop, dpop, year){
  ...
  pt2 <- ggplot(patches, aes(geometry=geometry, fill=infected)) + geom_sf() + theme_void() + ggtitle(str_c("Year ", year, " - ASF")) + theme(legend.pos="bottom") + scale_fill_manual(values=c(Extinct="Grey", No="Green", Yes="Red"))
  pt3 <- ggplot(patches, aes(geometry=geometry, fill=carcass)) + geom_sf() + theme_void() + ggtitle(str_c("Year ", year, " - Carcass")) + theme(legend.pos="bottom") + scale_fill_manual(values=c(No="Grey", Yes="chocolate3"))
  pt4 <- ggplot(patches, aes(geometry=geometry, fill=spillover)) + geom_sf() + theme_void() + ggtitle(str_c("Year ", year, " - Spillover")) + theme(legend.pos="bottom")
