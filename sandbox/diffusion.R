...
Diffusion <- R6Class("Diffusion",
	...
	public = list(
		...
		setup = function(graph, beta_freq, beta_dens) {
		  ...
		  (betas <- c(0.0, beta_dens/(4^(0:4)), 0.0))
		  stopifnot(length(betas)==7L)
		  # Populate migrate so it is a matrix of -betas based on distance:
		  private$beta_dens <- matrix(-betas[case_when(as.numeric(dist) > 5 ~ 7.0, TRUE ~ as.numeric(dist) + 1.0)], ncol=ncol(dist), nrow=nrow(dist))
